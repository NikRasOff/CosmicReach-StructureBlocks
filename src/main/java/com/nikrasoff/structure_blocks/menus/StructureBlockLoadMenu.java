...
public class StructureBlockLoadMenu extends BaseStructureBlockMenu {
    ...
    public final ElementSelectButton<Structure> structureInput;
    public StructureBlockLoadMenu(){
        super();

        this.createLabel(15, 130, 250, 50, "Structure:");
        this.structureInput = new ElementSelectButton<>(280, 130, 500, 50, HorizontalAnchor.LEFT_ALIGNED, VerticalAnchor.TOP_ALIGNED, this, Structure.ALL_STRUCTURES);
    ...
    protected void copyFromEntity() {
        ...
        if (!this.reflectedEntity.structureId.isEmpty()){
            this.structureInput.elementID = Identifier.fromString(this.reflectedEntity.structureId);
            this.structureInput.text = this.reflectedEntity.structureId;
            this.structureInput.updateText();
        }
        else {
            this.structureInput.elementID = null;
    ...
    protected void copyToEntity() {
        ...
        if (this.structureInput.elementID != null){
            this.reflectedEntity.structureId = this.structureInput.elementID.toString();
        }
    }
}
